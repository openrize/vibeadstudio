...
- Never invent testimonials, customer names, logos-as-endorsements, prices, discounts, guarantees, awards, certifications, or statistics that do not appear in the briefing.
...
/** @returns {{ ads: object[], brandIntel: object, usedAI: boolean }} */
export async function generateAds(scraped, count = 5) {
  ...
  if (hasAI()) {
    try {
      ...
      return { ads: merged, brandIntel, usedAI: true };
    } catch (err) {
...
- Business name: ${brandIntel.businessName || "Unknown"}
- Main service/product line: ${brandIntel.mainServiceProduct || "—"}
- Industry: ${brandIntel.detectedIndustry}
- Tone direction: ${brandIntel.toneDirection}
- Audience (from page + playbook): ${brandIntel.targetAudience}
- Suggested CTA vocabulary (prefer this posture; do not invent fake offers): ${(brandIntel.suggestedCtas || []).join(" | ")}
