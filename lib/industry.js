...
const RULES = [
  ...
  },
  {
    ...
    re: /(automotive|auto\s+dealer|dealership|test\s*drive|(?:car|truck|suv|vehicle|dealer|dealership)[^\n]{0,48}\binventory\b|\binventory\b[^\n]{0,48}(?:vehicles?|cars?|trucks?|suvs?|pre-?owned|certified)|\bsuv\b|\bsedan\b|\bcrossover\b|pickup\s+truck|car\s+dealer|bmw|mercedes|audi|lexus|cadillac|ford\s+motor|chevrolet|toyota\s+dealer)/i,
  },
  {
    key: "fashion",
  ...
  {
    key: "saas",
    re: /(saas|software\s+as\s+a\s+service|api\b|cloud\s+platform|free\s+trial|book\s+a\s+demo|integration|workflow\s+automation|subscription\s+plan|dashboard|\bdeploy(?:ment)?s?\b|serverless|developer\s+(?:platform|tools|experience)|\bci\s*\/\s*cd\b|continuous\s+integration|observability|kubernetes|\bcdn\b|edge\s+(?:network|computing)|infrastructure|frontend\s+cloud|build\s+and\s+deploy|preview\s+deploy|git\b|from\s+git|framework|sdk\b|cli\b|\/docs\b|documentation|(?:issue|project)\s+tracking|product\s+management|engineering\s+teams?|\broadmaps?\b|\bbacklogs?\b|\bworkflows?\b|sprint\s+planning|dev\s+tools|issue\s+tracker)/i,
  },
  {
    key: "ecommerce",
