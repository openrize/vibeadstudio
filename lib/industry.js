...
const RULES = [
  ...
  {
    key: "automotive",
    // Avoid bare "inventory" / "vehicle" — SaaS and commerce sites reuse those words (backlog inventory, etc.).
    // OEM tokens must be word-bounded: "audi" is a substring of "audience" and falsely triggered automotive.
    re: /(automotive|auto\s+dealer|dealership|test\s*drive|(?:car|truck|suv|vehicle|dealer|dealership)[^\n]{0,48}\binventory\b|\binventory\b[^\n]{0,48}(?:vehicles?|cars?|trucks?|suvs?|pre-?owned|certified)|\bsuv\b|\bsedan\b|\bcrossover\b|pickup\s+truck|car\s+dealer|\b(?:bmw|mercedes|audi|lexus|cadillac)\b|ford\s+motor|chevrolet|toyota\s+dealer)/i,
  },
  {
    key: "fashion",
  ...
  {
    key: "saas",
    re: /(saas|software\s+as\s+a\s+service|api\b|cloud\s+platform|free\s+trial|book\s+a\s+demo|integration|workflow\s+automation|subscription\s+plan|dashboard|\bdeploy(?:ment)?s?\b|serverless|developer\s+(?:platform|tools|experience)|\bci\s*\/\s*cd\b|continuous\s+integration|observability|kubernetes|\bcdn\b|edge\s+(?:network|computing)|infrastructure|frontend\s+cloud|build\s+and\s+deploy|preview\s+deploy|git\b|from\s+git|framework|sdk\b|cli\b|\/docs\b|documentation|(?:issue|project)\s+tracking|product\s+management|engineering\s+teams?|\broadmaps?\b|\bbacklogs?\b|\bworkflows?\b|sprint\s+planning|dev\s+tools|issue\s+tracker|(?:restaurants?|retail|hospitality|services?)\s+(?:management|operations|software|platform)|software\s+for\s+(?:restaurants?|retail|services?)|\bsystems?\s+(?:built\s+)?for\s+(?:restaurants?|retail|service)|vertical\s+saas|b2b\s+software)/i,
  },
  {
    key: "ecommerce",
...
export function buildBrandIntel(scraped, overrides = {}) {
  ...
  const automotiveLuxury = /\b(?:cadillac|lexus|mercedes|bmw|audi|lincoln|infiniti|acura|porsche|maserati)\b/i.test(
  ...
  const mainServiceProduct = inferMainServiceProduct(scraped);
  const targetAudience = inferTargetAudience(scraped, pb.audience);
  const benefitsSummary = inferBenefitsSummary(scraped);
  const suggestedCtas = (pb.ctas || []).slice(0, 4);
  ...
  return {
    ...
    targetAudience,
    audiencePlaybookNote: pb.audience,
    benefitsSummary,
    suggestedCtas,
...
function inferBusinessName(scraped) {
  const fromSite = cleanSnippet(scraped?.siteName);
  if (fromSite) return fromSite;
  const fromTitle = cleanSnippet(scraped?.title?.split(/[|\-–:]/)[0]);
  if (fromTitle) return fromTitle;
  try {
    const host = new URL(String(scraped?.url || "https://example.com")).hostname.replace(/^www\./, "");
...
function inferMainServiceProduct(scraped) {
  const hero = cleanSnippet(scraped?.heroHeadline);
  if (hero && hero.length > 8) return hero.slice(0, 240);
  const pn = cleanSnippet(scraped?.productNames?.[0]);
  if (pn) return pn.slice(0, 240);
  const desc = cleanSnippet(scraped?.description);
  if (desc && desc.length > 20) return desc.slice(0, 280);
  const svc = cleanSnippet((scraped?.serviceDescriptions || [])[0]);
  if (svc) return svc.slice(0, 240);
  return "—";
}

function inferTargetAudience(scraped, playbookAudience) {
  const desc = cleanSnippet(scraped?.description);
  if (desc.length >= 35) return desc.slice(0, 260) + (desc.length > 260 ? "…" : "");
  const sub = (scraped?.subheadings || []).map(cleanSnippet).find((s) => s.length > 25);
  if (sub) return sub.slice(0, 220);
  const para = (scraped?.paragraphs || []).map(cleanSnippet).find((p) => p.length > 70);
  if (para) return para.slice(0, 260) + (para.length > 260 ? "…" : "");
  return playbookAudience;
}

function inferBenefitsSummary(scraped) {
  const lines = [
    ...(scraped?.benefits || []),
    ...(scraped?.featureBullets || []).slice(0, 4),
  ]
    .map(cleanSnippet)
    .filter(Boolean);
  const unique = [];
